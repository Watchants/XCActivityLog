...
typedef enum XCActivityLogItemType {
    XCActivityLogItemEnded = 0,
    XCActivityLogItemInteger = 1,
    XCActivityLogItemFloatingPoint = 2,
    XCActivityLogItemNil = 3,
    XCActivityLogItemString = 4,
    XCActivityLogItemArray = 5,
    XCActivityLogItemClassName = 6,
    XCActivityLogItemClassInstance = 7
} XCActivityLogItemType;

typedef struct XCActivityLogItem {
    const char * payload;
    char delimiter;
    XCActivityLogItemType type;
    uint64_t number;
    double floatingPoint;
    const char * text;
} XCActivityLogItem;

typedef const XCActivityLogItem * XCActivityLogItemRef;

extern XCActivityLogItemRef XCActivityLogItemCreate(const void * scannerRef, const char * payload, char delimiter);

extern void XCActivityLogItemRelease(XCActivityLogItemRef thisRef);
